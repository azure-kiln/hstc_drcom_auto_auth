...
var page_data_encrypt='0'; //页面传输数据是否加密 0-不开启,1-开启

var encryption_type='1'; // 数据加密类型 0-ip,1-密钥(默认ip)

var secret_key='test';  //数据加密密钥(如果数据加密类型使用为1则使用)
...
var page = {
  ...
  checkIsGroupScanQRCode: function () {
	...
	/*
	else if ((term.redirect && term.redirect.indexOf('wechat_redirect') >= 0) || window.location.hash == '#wechat_redirect') {
      window.location = 'a70.htm' + url;
      return true;
	}
	*/
	else {
  ...
  firstRender: function () {
    ...
    else if ((term.redirect && term.redirect.indexOf('dingtalk_redirect') >= 0) || window.location.hash == '#dingtalk_redirect') {
    ...
    util._load('js', page.page_asset + 'js/all.js', function () {
      ...
      util._load('js', page.page_asset + 'js/jquery.i18n.js', function() {
        ...
        if (isJSMin) {
           ...
           */
          var jsFiles = ['a42.js', 'a43.js', 'a44.js', 'a45.js', 'a47.js', 'a48.js', 'a49.js', 'a50.js', 'a51.js','a52.js','a58.js','a60.js','a59.js'];
...
var util = {
  ...
  _jsonp: function (params) {
    ...
    window.onerror = function (message, source, lineno, colno, error) {
      // 尝试捕获非JSONP响应的语法错误(认证操作进行处理，其他业务跳过)
    ...
    // 处理JSONP错误的通用函数
		function handleJsonpError() {
			...
			if (params.data && params.data['operate'] === "portal_login") {
				try {
					console.log("检测到 portal_login 操作失败，执行错误处理");
					...
					console.error("handleJsonpError 处理出错:", e);
				}
			} else {
				console.log("非 portal_login 操作，跳过错误处理");
			}
		}
  ...
  */
  base64Encode: function (input) {
    var base64Chars = this.base64EncodeChars;
    ...
    if (chunks[chunks.length - 1].length < 6) {
      chunks[chunks.length - 1] = chunks[chunks.length - 1].padEnd(6, '0');
		...
		// 查找对应的 Base64 字符
    var base64Encoded = chunks.map(chunk => {
		  var index = parseInt(chunk, 2); // 将二进制转换为数字
  ...
  */
  base64Decode: function (input) {
    ...
    for (var i = 0; i < input.length; i++) {
      var index = base64Chars.indexOf(input.charAt(i)); // 查找字符的位置
    ...
    // 将字节数组转换为 UTF-8 字符串
    var decodedString = new TextDecoder().decode(new Uint8Array(decodedBytes));
...
var term = {
  ...
  authex_enable: '',//选泽参数
	temp_acc:'',//账号临时存储
  is_base64_encode: 1, // 启用Base64编码兼容账号/密码中特殊字符
